...
def generate_and_filter_iconclass(classes, iconclass_data):
    ...
    for iconclass in filtered_iconclasses:
        ...
        extend_all_parents.extend([x.code for x in Notation(iconclass).get_parents_until()])
    ...
    for iconclass in extend_all_parents:
        old_annotation.append({iconclass: {"p": [x.code for x in Notation(iconclass).get_parents_until()]}})
...
def generate_iconclass_trace(iconclass_annotation):
    ...
    results.extend([x.code for x in notation.get_parents_until()])
    results.append(notation.code)
            ...
            for label in sample:
                ...
                for s in sample_iconclasses:
                    ...
                    if s in iconclass_mapping:
                        ...
                        try:
                            for x in Notation(s).get_parents_until()[::-1]:
...
def main():
    ...
    level_stat = {}
    first_stat = {}
    for i, iconclass in enumerate(classes):
        print(i)
        for x in generate_iconclass_trace(iconclass):
            p = len(x.get_parents_until())
            if p not in level_stat:
                level_stat[p] = 0
            level_stat[p] += 1

            if p == 0:
                if x.code not in first_stat:
                    first_stat[x.code] = 0
                first_stat[x.code] += 1

            else:
                c = list(x.get_parents_until())[0].code
                if c not in first_stat:
                    first_stat[c] = 0
                first_stat[c] += 1

            # print(len(x.get_parents_until()))

    print(level_stat)
    print(first_stat)
    #     all_iconclasses.extend()
