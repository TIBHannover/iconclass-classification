    ...
    with open(args.output_path, "w") as file:
        ...
        for possible_prediction in possible_predictions:
            ...
            for f in args.filter:
                # print(f)
                # print(len(mapping_config))
                train_mask = gen_filter_mask(mapping_config, f)
                # print(train_mask)
                # print(train_mask.shape)
                # print(np.sum(train_mask * ap_mask))
                # print(np.sum(ap_mask))
                score = np.sum(np.nan_to_num(ap) * ap_mask * train_mask) / np.sum(ap_mask * train_mask)
                num_classes = np.sum(train_mask * ap_mask)
                ...
                for x in range(max_level):
                    level_mask = (level_map == x).astype(np.int64)

                    score = np.sum(np.nan_to_num(ap) * ap_mask * train_mask * level_mask) / np.sum(
                        ap_mask * train_mask * level_mask
                    )
                    num_classes = np.sum(train_mask * level_mask * ap_mask)
                    print(f"\tf: {f}, {score} {num_classes}")
                    file.write(
                        json.dumps(
                            {
