...
def parse_args():
    ...
    parser.add_argument("-i", "--image_path", required=True, help="path to the prediction h5 file")
    parser.add_argument("-a", "--annotation_path", required=True, help="path to the prediction h5 file")
...
def main():
    ...
    annotation_dict = read_line_data(args.annotation_path, dict_key="id")
    ...
    top_20_list = []
    with open(args.prediction_path) as file:
        for line in file:
            d=json.loads(line)
            num_gt = sum([x["target"] for x in d["top_20"]])
            if d["score"] > 0.5 and num_gt > 5:
                top_20_list.append(d)

    selection = random.sample(top_20_list, 5)

    for s in selection:
        m = index_mapping_lut[s["index"]]
        output_path = os.path.join(args.output_path, m["id"])
        os.makedirs(output_path, exist_ok=True)
        for i,sample in enumerate(s["top_20"]):
            a = annotation_dict[sample["id"]]
            old_path = os.path.join(args.image_path, a["rel_path"])
            if not os.path.exists(old_path):
                print(old_path)
                continue
            new_path = os.path.join(output_path, f"{i}_{sample['target']}_{sample['prob']}.jpg")
            shutil.copyfile(old_path, new_path)

        print(m["id"], [x["target"] for x in s["top_20"]])

    print(len(top_20_list))
    ...
    #         score = np.nan_to_num(ap) * ap_mask
    #         print(score.shape)
    #         for i, s in enumerate(np.nditer(score)):
    #             print(i)
    #             if s > 0.0:
    #                 file.write(json.dumps({"score": s.item(), "index": i, "top_20": [{"id":ids[x].decode(), "target":target[x,i].item(), "prob":prediction[x,i].item()} for x in np.nditer(np.argsort(-prediction[:,i])[:20])]})+"\n")

            # np.random.choice(range(), size=5, replace=False)
